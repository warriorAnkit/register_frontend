...
const TableFieldComponent = forwardRef(
  (
    ...
    const [deleteRow]=useMutation(DELETE_FIELD_RESPONSE_ROW);
    const [storeRowResponseInJsonb] = useMutation(STORE_ROW_RESPONSE_IN_JSONB);
    const [editRowResponseInjsonb]=useMutation(UPDATE_FIELD_ROW_RESPONSES);
...
console.log("filling",filling);
    ...
    const checkRowCompletionStatus = () => {
      ...
      const eachRowCompletionStatus = [];
      tableData.forEach((row, rowIndex) => {
        if (rowIndex === tableData.length - 1) {
          eachRowCompletionStatus[rowIndex] = true;
          return;
        }
        ...
        if (!isRowComplete) {
          eachRowCompletionStatus[rowIndex] = false;
      ...
      setIsAllRowsComplete(allRowsComplete);
    };
    useEffect(() => {
      checkRowCompletionStatus();
    }, [tableData]);
      ...
      if (isRowComplete) {
        try {
          const tableEntriesiess = Object.entries(row)
            ...
            .filter((entry) => entry.fieldId && entry.value);

          if (!row.rowNumber) {
            const rowResponse = await storeRowResponseInJsonb({
              variables: {
                setId,
                templateId,
                tableEntries: tableEntriesiess,
                filling,
              },
            });
            if (rowResponse.data.storeRowResponseInJsonb.success) {
              const {rowNumber} = rowResponse.data.storeRowResponseInJsonb;

        const updatedData = [...tableData];
        updatedData[rowIndex] = { ...row, rowNumber };
        setTableData(updatedData);
            } else {
              console.error(
                'Error storing row:',
                rowResponse.data.storeRowResponseInJsonb.message,
              );
            }
          } else {
            const rowResponse = await editRowResponseInjsonb({
              variables: {
                setId,
                templateId,
        ...
        templateData.getTemplateById?.fields.forEach((field) => {
          ...
          if (
            ...
            if (!numericRegex.test(value)) {
              errors[`${field.id}-${rowIndex}`] =
                'Value must be a valid number with up to 15 digits before the decimal and up to 2 digits after.';
            }
          }
        });
      });
      ...
      return (
        ...
        >
          ...
          {fieldType === 'OPTIONS' && (
            <Select
              ...
              onDropdownVisibleChange={(open) =>
                handleOpenChange(open, rowIndex, fieldName)
              }
          ...
          {fieldType === 'CALCULATION' && (
            <Input
              value={
                rowIndex <= tableData.length - 1 ? calculateFieldValue() : ''
              }
              disabled
              style={{
                width: '100%',
        ...
        onOk: async () => {
          try {
            if (row.rowNumber) {
              ...
              if (response.data.deleteFieldResponseRow.success) {
