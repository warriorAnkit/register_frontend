...
const ImageUpload = ({ onUploadSuccess, errorMessage, existingFileUrls }) => {
  ...
  const beforeUpload = (file,fileList) => {
    ...
    const uploadedFileCount = uploadedFileUrls ? uploadedFileUrls.split(',').length : 0;
    if (fileList.findIndex(f => f === file) > (2-uploadedFileCount)) {
  ...
  console.log("ankit",uploadedFileUrls.split(',').length,"dd",uploadedFileUrls);
  return (
    <div>
